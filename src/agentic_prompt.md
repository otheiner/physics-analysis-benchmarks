...
All input files are listed above by name and are available in your working directory — their contents are not pre-loaded. Use `read_file` or `run_command` to inspect text and CSV files{view_image_note}. Files written by `execute_python` or `write_file` persist between tool calls, but the working directory always resets to the workspace root — `cd` changes do not carry over between calls. Use explicit paths or chain `cd` within a single call (e.g. `cd subdir && head file.csv`).
